...
import type { RecoverPageContent } from "@/lib/landingPageSchemas";
        ...
        <div>
          ...
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: c.hero.subheadline
                .replace(/\$166 billion/g, '<strong class="text-white">$166 billion</strong>')
                .replace(/83% haven't filed yet\./g, '<strong class="text-white">83% haven\'t filed yet.</strong>'),
            }}
          />
...
function HowItWorksSection({ c }: { c: RecoverPageContent }) {
  return (
...
const DEFAULT_ORDER = ["hero", "howItWorks", "resources", "footer"];
...
export default async function RecoverPage({
  ...
  const order = c._sectionOrder ?? DEFAULT_ORDER;

  function renderSection(id: string) {
    switch (id) {
      case "hero":
        return <HeroSection key="hero" c={c} partnerCode={partnerCode} utmParams={utmParams} />;
      case "howItWorks":
        return <HowItWorksSection key="howItWorks" c={c} />;
      case "resources":
        return <GatedResources key="resources" partnerCode={partnerCode} />;
      case "footer":
        return <FooterSection key="footer" c={c} />;
      default:
        return null;
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#060a14] text-white">
      {/* Nav — always fixed at top, not subject to section ordering */}
      <nav className="border-t border-white/5 border-b">
