...
// Reordered variant of RecoverForm: asks the IEEPA duties amount paid in 2025
// (original question 2) BEFORE the import-category question (original question 1).
export default function RecoverFormReordered({ partnerCode, utmParams }: Props) {
  const [step, setStep] = useState<"duties" | "product" | "timing" | "result" | "contact" | "not_qualified" | "done">("duties");
  ...
  const progress = step === "duties" ? 1 : step === "product" ? 2 : step === "timing" ? 3 : step === "result" ? 4 : step === "contact" ? 5 : 5;
  ...
  return (
    <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 sm:p-8 backdrop-blur-sm">
      ...
      {/* Step 1: How much in duties? (asked first, framed for 2025) */}
      {step === "duties" && (
        <>
          <h2 className="font-display text-xl mb-1" style={{ color: "var(--brand-gold)" }}>How much in IEEPA duties did you pay in 2025?</h2>
          <p className="text-sm text-white/40 mb-5">Enter your total IEEPA tariff duties paid in 2025 (not total import value).</p>
          ...
          <button
            onClick={() => dutiesAmount > 0 && setStep("product")}
            disabled={dutiesAmount <= 0}
            className="w-full py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-30"
        ...
        <>
          ...
          <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
            {HTS_CATEGORIES.map((c) => (
              <button
                key={c.code}
                onClick={() => { setSelectedCategory(c); setStep("timing"); }}
          ...
          <button onClick={() => setStep("duties")} className="mt-4 text-xs text-white/40 hover:text-white/60">← Back</button>
        </>
      )}
      ...
      {step === "timing" && (
        <>
          ...
          <button onClick={() => setStep("product")} className="text-xs text-white/40 hover:text-white/60">← Back</button>
        </>
      )}
