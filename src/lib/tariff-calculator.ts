 ...
 *  - cape_phase1: unliquidated OR liquidated within the 80-day window → automated CAPE refund (standard entries)
 *  - cape_phase2: reconciliation-flagged entries (Types 01/02/06, Type 09 not yet filed) — CAPE Phase 2,
 *                 launched June 29, 2026; same 80-day liquidation window applies
...
export interface EntryForEligibility {
  ...
  isReconciliationFlagged?: boolean; // entry (Type 01/02/06) flagged for reconciliation with no Type 09 filed → CAPE Phase 2 (eff. June 29, 2026)
...
export function checkEligibility(entry: EntryForEligibility): EligibilityResult {
  ...
  // Reconciliation-flagged entries (Types 01/02/06, no Type 09 filed) → CAPE Phase 2 (eff. June 29, 2026)
  const automatedCapeMethod: FilingMethod = entry.isReconciliationFlagged ? "cape_phase2" : "cape_phase1";
  ...
  if (entry.liquidationDate) {
    ...
    // Within 80 days of liquidation → automated CAPE (Phase 1 or Phase 2); 80–180 days → formal protest
    const filingMethod: FilingMethod =
      daysSinceLiquidation <= CAPE_PHASE1_LIQUIDATION_WINDOW_DAYS ? automatedCapeMethod : "protest";
    ...
    const base: EligibilityResult = {
      ...
      reason:
        filingMethod === "cape_phase1"
          ...
          ? "Liquidated within 80 days — eligible via CAPE Phase 2 (reconciliation-flagged entry)"
  ...
  const base: EligibilityResult = {
    status: "eligible",
    reason: entry.isReconciliationFlagged
    ...
    filingMethod: automatedCapeMethod,
  };
  return applySectionReviewFlag(base, entry);
}
