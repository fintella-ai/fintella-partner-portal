...
export const TRIGGER_LABELS: Record<TriggerKey, string> = {
  ...
  "partner.onboarding_stalled": "Onboarding Stalled (scheduled)",
...
export const TRIGGER_VARIABLES: Record<TriggerKey, TriggerVariable[]> = {
  "deal.created": [
    ...
    { token: "{deal.idempotencyKey}",        description: "Idempotency / dedup key the partner sent on creation (may be empty)", example: "frost-hs-462693304018" },
    ...
    { token: "{deal.businessState}",         description: "Client business state (full name)",      example: "Arizona" },
    { token: "{deal.businessZip}",           description: "Client business zip / postal code",      example: "85004" },
    { token: "{deal.businessAddress}",       description: "Full formatted business address (all lines combined)", example: "123 Commerce Blvd, Suite 400, Phoenix, Arizona 85004" },
    ...
    { token: "{deal.stage}",                 description: "Internal stage",                         example: "lead_submitted" },
    ...
    { token: "{deal.internalRawCode}",       description: "Internal raw service code (passed by partner at submission, if any)", example: "SVC-TARIFF-001" },
  ],
  "deal.stage_changed": [
    { token: "{deal.dealName}",              description: "Deal name",                  example: "ACME Corp — Tariff Refund" },
    { token: "{deal.stage}",                 description: "New internal stage (same as newStage)", example: "meeting_booked" },
    { token: "{previousStage}",              description: "Internal stage before the change",      example: "lead_submitted" },
    { token: "{newStage}",                   description: "Internal stage after the change",       example: "meeting_booked" },
    { token: "{deal.partnerCode}",           description: "Submitting partner's code",             example: "PTNJD8K3F" },
    { token: "{deal.referralPartnerName}",   description: "Submitting partner's display name (firstName + lastName)", example: "Jane Doe" },
    { token: "{deal.dealUrl}",               description: "Direct admin portal link to this deal", example: "https://fintella.partners/admin/deals#cmoabkqqi000e14ab8ybk8bv4" },
  ...
  "partner.onboarding_stalled": [
    { token: "{partner.partnerCode}",        description: "Partner code of the stalled partner",                         example: "PTNJD8K3F" },
    { token: "{partner.firstName}",          description: "First name",                                                  example: "Jane" },
    { token: "{partner.lastName}",           description: "Last name",                                                   example: "Doe" },
    { token: "{partner.email}",              description: "Email address",                                               example: "jane@firm.com" },
    { token: "{partner.mobilePhone}",        description: "Mobile phone in E.164 (may be empty)",                        example: "+14105551234" },
    { token: "{checklist.completedCount}",   description: "Number of steps completed",                                   example: "3" },
    { token: "{checklist.totalCount}",       description: "Total steps in the checklist (currently 9)",                  example: "9" },
    { token: "{checklist.progressPercent}",  description: "Progress percent (0-100)",                                    example: "33" },
    { token: "{nextStep.title}",             description: "Title of the next ready step",                                example: "Complete your profile" },
    { token: "{nextStep.description}",       description: "One-sentence explanation of the next step",                   example: "Add your address so we can generate accurate tax forms." },
    { token: "{nextStep.ctaUrl}",            description: "Deep link to the next step's feature",                        example: "/dashboard/settings?tab=address" },
    { token: "{daysSinceSignup}",            description: "Whole days since signup",                                     example: "12" },
  ],
  "partner.added_to_channel": [
