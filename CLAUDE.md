...
- **Phase 15**: Email, SMS & VOIP Integration
  ...
  - **Phase 15b — SMS (Twilio Programmable Messaging)**: opt-in partner SMS notifications, logged in partner communication log
  ...
  - **Integration plan (15b/15c)**: Twilio account → provision phone numbers → build `/api/twilio/call` endpoint for outbound calls → `/api/twilio/sms` for sending → `/api/twilio/webhook` for status callbacks → CallLog + SmsLog Prisma models → display in partner communication log
