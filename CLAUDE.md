...
- **Public marketing OLED theme** (opcenter.app/launch look — shipped #1101/#1102/#1103): the public marketing pages (`landing-v2`, `apply`, `pricing`, `partners/brokers`, `webinar`, `calculator`, `recover/tariff-diy`) use a **scoped `.oc-launch`** block in `globals.css` that **re-points `--app-*`/`--brand-gold` to OLED values (near-black `#050507`, violet `#7c3aed`) LOCALLY** — token-driven markup re-skins automatically; the global `:root` defs (backoffice) are NEVER edited. Reusable kit in `src/components/marketing/` (MarketingShell, MarketingAtmosphere, Eyebrow, GradientText, GlassCard, StatCard, FeatureIcon) + utilities `.oc-gradient-text`/`.oc-eyebrow`/`.oc-glass`/`.oc-feature-icon`/`.oc-cta`. Per-page recipe + full pattern: `docs/knowledge/opcenter-theme-tokens.md`. The authenticated backoffice is **untouched** — apply the same scoped approach behind `data-portal-theme="opcenter"` when rolling it out.
- **CSS-comment gotcha**: never write `*/` inside a CSS comment (e.g. `--foo-*/`) — it closes the comment early and breaks the **app-wide** compile. CI "Vercel SUCCESS" will NOT catch it (this project **cancels all preview deployments**); only a local `next dev` + screenshot will. Verify marketing UI with `./node_modules/.bin/next dev` + Playwright, never assume the green check means it renders.
