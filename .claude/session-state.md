...
🕒 Last updated: 2026-04-23 — PRs #435 + #436 merged: hardened the Google Calendar OAuth redirect URI. #435 trimmed whitespace from NEXT_PUBLIC_PORTAL_URL (the env var had a trailing newline from `echo | vercel env add` that produced "fintella.partners /api/..." with a space, which Google's redirect_uri check rejected). #436 then swapped the env-var source for req.nextUrl.origin in both oauth-start and oauth-callback so the URL comes from the actual request host — bypasses the polluted env entirely. Admin still needs to click Connect once on /admin/settings → Integrations tab to complete the OAuth dance. PR #433 earlier in the session shipped the full OAuth integration. Earlier today: #404 layout padding, #406 Commission History columns, #408 EP override card + sub-tab, #410 agreement + invite reminders, #412 HubSpot-style workflow editor, #414 Live Weekly seed gate, #416 Jitsi embed + conference reminders, #418 admin downline list shows L3s, #420 Top Partners deals counts override deals, #422 delete-error surfacing + jitsiRoom backfill, #424 drop conference demo fallbacks, #425 hide orphaned ledger rows, #427 Jitsi slug field, #429 after-call reminder tip, #431 URL column on admin conference table.

## 🌿 Git state
- **main HEAD:** `b4186cb` — fix(oauth): derive redirect_uri from request origin, not env var (#436)
- **origin/main:** in sync
