...
async function main() {
  ...
  const emailTemplates = [
    ...
    // ── Broker Cold Email Outreach (3-step sequence) ──────────────────────
    // Targets 14,454 customs broker contacts. Each template is wired to
    // CampaignStep rows via templateKey. The campaign-engine.ts also has
    // hardcoded fallbacks for these keys.
    {
      ...
      ctaUrl: "https://fintella.partners/partners/brokers?utm_source=email&utm_medium=outreach&utm_campaign=broker-cold-email&utm_content=PTNS4XDMN",
      ...
      isDraft: false,
      description:
        "Cold email 1 of 3 for broker outreach campaign. Value prop — IEEPA refund opportunity. Sent as step 1 of the 'Broker Cold Outreach v2' campaign via campaign-engine.ts drip processor.",
      variables: JSON.stringify([
        "firstName",
        "lastName",
        "location",
    ...
    },
    {
      ...
      isDraft: false,
      description:
        "Cold email 2 of 3 for broker outreach campaign. Widget demo + commission math. Sent 3 days after email 1 via campaign-engine.ts drip processor.",
      variables: JSON.stringify([
        "firstName",
        "lastName",
        "location",
    ...
    },
    {
      ...
      bodyText:
        ...
        "Become a partner: {ctaUrl}\n\n" +
        "Best,\nFintella Partner Team",
      ctaLabel: "Become a Partner",
      ctaUrl: "https://fintella.partners/partners/brokers?utm_source=email&utm_medium=outreach&utm_campaign=broker-cold-email&utm_content=PTNS4XDMN#signup-form",
      fromEmail: "outreach@fintella.partners",
      fromName: "Fintella",
      replyTo: "outreach@fintella.partners",
      enabled: true,
      isDraft: false,
      description:
        "Cold email 3 of 3 for broker outreach campaign. Urgency play — 180-day deadline + penalties. Sent 3 days after email 2 via campaign-engine.ts drip processor.",
      variables: JSON.stringify([
        "firstName",
        "lastName",
        "location",
  ...
  for (const v of TEMPLATE_VARS) {
    await prisma.templateVariable.upsert({ where: { key: v.key }, update: v, create: v });
  }
  console.log("  ✓ " + TEMPLATE_VARS.length + " template variables");
  ...
  for (const s of COMM_STYLES) {
    const ex = await prisma.communicationStyle.findFirst({ where: { name: s.name } });
    if (!ex) await prisma.communicationStyle.create({ data: s });
    else await prisma.communicationStyle.update({ where: { id: ex.id }, data: s });
  }
  console.log("  ✓ " + COMM_STYLES.length + " communication styles");
  ...
  for (const a of WF_ACTIONS) {
    await prisma.workflowAction.upsert({ where: { tag: a.tag }, update: a, create: a });
  }
  console.log("  ✓ " + WF_ACTIONS.length + " workflow actions");
